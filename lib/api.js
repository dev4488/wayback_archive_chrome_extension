 ...
 * @param {string} [engine='wayback']
 * @returns {Promise<{ url: string, timestamp: string }|null>}
 */
export async function fetchLatestSnapshot(url, skipCache = false, engine = 'wayback') {
  const normalized = normalizeUrl(url);

  if (engine === 'archiveIs') {
    return {
      url: `https://archive.is/newest/${encodeURI(normalized)}`,
      timestamp: 'archiveis_mock'
    };
  }
...
/**
 * Request the Wayback Machine or Archive.is to save/archive a URL.
 * @param {string} url
 * @param {string} engine ('wayback' or 'archiveIs')
 * @returns {Promise<{ success: boolean, archiveUrl?: string, error?: string, newTabRequired?: boolean }>}
 */
export async function saveToArchive(url, engine = 'wayback') {
  const normalized = normalizeUrl(url);

  if (engine === 'archiveIs') {
    return {
      success: true,
      archiveUrl: `https://archive.is/?run=1&url=${encodeURIComponent(normalized)}`,
      newTabRequired: true
    };
  }
...
export function formatTimestamp(timestamp) {
  if (timestamp === 'archiveis_mock') return 'Archive.is page';
